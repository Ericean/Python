...
class GuiPart:
    ...
    #Add the click event to the Queue2 for data processing in ThreadedClient
...
class ThreadedClient:
    ...
    def workerThread1(self):
        ...
        while self.running:
            ...
            # You can replace the following lines with the real stuff
            try:
                  buffer_size=4096
                  raw_bytes = self.s.recv(buffer_size)
            except socket.error as msg:
            ...
            if not self.string:#Check whether the data is empty
                break
            print("Received %d bytes from server" % len(raw_bytes))#runs background
            print("Message contents: %s" % self.string)#runs background
            self.queue.put(self.string)#Pass the received data to the GUI to display
